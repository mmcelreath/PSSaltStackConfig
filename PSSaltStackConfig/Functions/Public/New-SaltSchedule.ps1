...
function New-SaltSchedule {
    ...
    $schedule = Get-SaltSchedule -SaltConnection $SaltConnection -Name $Name -ExactMatch

    if ($schedule.Count -gt 0) {
        throw "Another Schedule exists already with the name: $Name"
    } 

    $job = Get-SaltJob -SaltConnection $SaltConnection -Name $JobName -ExactMatch
    $target = Get-SaltTarget -SaltConnection $SaltConnection -Name $TargetName -ExactMatch
    ...
    $return = Invoke-SaltStackAPIMethod -SaltConnection $SaltConnection -Resource schedule -Method save -Arguments $arguments
